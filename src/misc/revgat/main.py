...
def load_data(dataset, args):
    global n_node_feats, n_classes, feat_static
    ...
    if args.use_bert_x:
        ...
        feat_static = torch.load(args.bert_x_dir)
        logger.warning(
            "Loaded pre-trained node embeddings of shape={} from {}".format(feat_static.shape, args.bert_x_dir)
        )
    # TODO
    ...
    if args.use_gpt_preds:
        ...
        n_node_feats = feat_static.shape[1]
...
def preprocess(graph):
    global n_node_feats, feat_static

    # make bidirected
    # feat = graph.ndata["feat"]
    graph = dgl.to_bidirected(graph)
    
    graph.ndata["feat"] = feat_static
...
def train(
    ...
    global feat_static, device
    feat = feat_static.to(device)
    model.train()
    if mode == "student":
...
def main():
    global device, n_node_feats, n_classes, epsilon, feat
    set_logging()
    args = parse_args()
