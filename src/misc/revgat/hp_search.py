...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running, trial):
...
def parse_args():
    ...
    argparser.add_argument("--backbone", type=str, default="rev", help="gcn backbone [deepergcn, wt, deq, rev, gr]")
    argparser.add_argument("--group", type=int, default=2, help="num of groups for rev gnns")
    ...
    argparser.add_argument("--use_bert_x", action="store_true", help="use bert x")
    argparser.add_argument("--bert_x_dir", type=str, help="bert x dir")
...
def set_hp_search_space(args, trial):
    args.n_hidden = trial.suggest_categorical("n_hidden", [128, 256, 512])
    args.n_layers = trial.suggest_categorical("n_layers", [2, 3])
    args.n_heads = trial.suggest_categorical("n_heads", [2, 4])
    args.dropout = trial.suggest_float("dropout", 0.3, 0.9)
    args.input_drop = trial.suggest_float("input_drop", 0.1, 0.7)
    args.edge_drop = trial.suggest_float("edge_drop", 0.1, 0.7)
    args.use_labels = trial.suggest_categorical("use_labels", [True, False])
    args.n_label_iters = trial.suggest_categorical("n_label_iters", [1, 2])
    args.group = trial.suggest_categorical("group", [1, 2])
    args.label_smoothing_factor = trial.suggest_float("label_smoothing_factor", 0.0, 0.8)
    return args
...
def objective(trial):
    ...
    for i in range(args.n_runs):
        seed(args.seed + i)
        val_acc, test_acc = run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, i + 1, trial)
